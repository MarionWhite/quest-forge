#!/usr/bin/env python3
"""Quest Forge loading chant v5: v3/v4 chant bed, only the two approved anvil timbres.

v4 excerpt listen: the 1st and 3rd isolated examples were liked; the others are dropped.

    Excerpt #1 @ 0.557s  -> BSB 7.080  medium-heavy  (v4 loop 11.18)
    Excerpt #3 @ 7.650s  -> BSB 26.030 light         (v4 loop 23.17)

Rejected excerpt #2 / #4 / #5 (BSB 0.139 heavy, 23.252 follow, 21.937 far light)
are not used. Same strike may be reused at a different gain/distance so the loop
is not two copy-pastes.

Pattern: hit1, rest, hit3 then hit1 closer (cluster), rest, hit3 farther.
Irregular, off the 72 bpm grid, no hits in the first/last ~8 s, distant under
the chant. Same circular-stereo / 8 s RMS-clipped OLA / micro-glue seam as v4.

Regen: python3 chant_v5.py
"""
import os
import subprocess
import numpy as np
...
# Approved excerpt examples only (order of appearance in anvil_v4_excerpt).
HIT1_SRC = 7.080    # excerpt #1, medium-heavy
HIT3_SRC = 26.030   # excerpt #3, light

# Final-loop times. Irregular smith: single, long rest, cluster, long rest, far.
# 72 bpm grid is 0.833 s; none of these land on a beat.
ANVIL_HITS = (11.42, 22.67, 23.19, 37.04)
# source time, velocity, distance (0 = closer, 1 = farther across the keep)
ANVIL_VOICES = (
    (HIT1_SRC, 0.94, 1.02),   # hit1 single
    (HIT3_SRC, 0.78, 0.94),   # hit3 cluster lead
    (HIT1_SRC, 1.00, 0.80),   # hit1 closer follow (~520 ms)
    (HIT3_SRC, 0.66, 1.20),   # hit3 farther after a long rest
)
ANVIL_GAIN = 0.132

# Isolated excerpt: same four voices, compressed rests, cluster gap kept.
EXCERPT_AT = (0.55, 4.40, 4.92, 8.80)
EXCERPT_DUR = 12.4
...
def judge_hit(name, y):
    """Concrete gates vs a real hammered anvil."""
...
def main():
    ...
    print("-- approved excerpt map --")
    print("   excerpt #1 @ 0.557s = BSB %.3f medium-heavy" % HIT1_SRC)
    print("   excerpt #3 @ 7.650s = BSB %.3f light" % HIT3_SRC)
    print("   dropped excerpt #2/#4/#5 (BSB 0.139 / 23.252 / 21.937)")
    print("-- v5 voices (only those two sources) --")
    for i, ((src_t, vel, far), loop_t) in enumerate(zip(ANVIL_VOICES, ANVIL_HITS)):
        kind = "hit1" if abs(src_t - HIT1_SRC) < 1e-6 else "hit3"
        print("   %s  loop %.2fs  src %.3f  vel %.2f  far %.2f" % (kind, loop_t, src_t, vel, far))

    print("-- v5 isolated-hit gates (want att<6ms, eSub<0.12, rPing>0.45, ringC>1400) --")
    ok = True
    for i, d in enumerate(distant):
        ok = judge_hit("hit%d@%.2f src%.3f" % (i, ANVIL_HITS[i], ANVIL_VOICES[i][0]), d) and ok

    iso = np.zeros(int(EXCERPT_DUR * SR))
    for h, at, (_src, vel, _far) in zip(distant, EXCERPT_AT, ANVIL_VOICES):
        n = min(len(h), len(iso) - int(at * SR))
        iso[int(at * SR) : int(at * SR) + n] += h[:n] * vel
    iso *= 0.78 / (np.max(np.abs(iso)) + 1e-9)
    write_audio("anvil_v5_excerpt", circular_stereo(iso))
    ...
    report("v5 loop", looped)
    anvil_vs_chant(looped, ANVIL_HITS)

    loop_dur = len(looped) / float(SR)
    for t in ANVIL_HITS:
        if t < 8.0 or t > (loop_dur - 8.0):
            print("WARNING anvil at %.2f is in the first/last 8s" % t)
        if t < XF_SEC + 2.0 or t > (loop_dur - XF_SEC - 2.0):
            print("WARNING anvil at %.2f is too close to the seam" % t)

    write_audio("questforge_loading_chant_v5", looped)
    two = np.concatenate([looped, looped], 0)
    mid = len(looped)
    write_audio("questforge_loading_chant_v5_seam_excerpt", two[mid - int(8 * SR) : mid + int(8 * SR)])
    a, b = int(8.5 * SR), int(40.5 * SR)
    write_audio("questforge_loading_chant_v5_anvil_context", looped[a:b])

    if not ok:
        print("GATES FAILED — do not install over v4 splash")
    else:
        print("GATES PASSED — v5 stem uses only excerpt #1 and #3 timbres")
